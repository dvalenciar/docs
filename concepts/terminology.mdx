...
2. **Action-Value Function** (or **Q-value**) $\text\{Q-value}$: This estimates the expected return if the agent takes a specific action $a$ from state $s$ and then follows a policy $\pi$.